...
auto Input::update(const float deltaTime) -> void {
...
auto Input::isKeyDown(const Key key) -> bool {
    return m_currentKeyboardState.at(key);
}
...
auto Input::wasKeyDown(const Key key) -> bool {
    return m_previousKeyboardState.at(key);
}
...
auto Input::processKey(const Key key, const bool isPressed) -> void {
    KeyboardState::iterator it { m_currentKeyboardState.find(key) };
